...
$id = 'dd-case-studies-item-' . $block['id'];
...
@endforeach
@endif
  <div class="dd-case-studies__items__item dd-case-studies__items__item--visible {{ $category_classes }}">
    <h3>{{ $title }}</h3>
    <img src="{{ $image }}" alt="{{ $title }}">
    <a class="inline-block mt-4 text-black font-proxima" href="#">{{ __('View ', 'sage') . $category_singular_name }}</a>
  </div>
@endwhile
</div>
</section>
...
(function($) {
  $('.dd-case-studies__items__item:odd').each(function (i) {
  ...
  $(function(){
    $('.dd-case-studies__filter__link').click(function(){
      ...
      var items = $('.dd-case-studies__items__item');
      var match = items.filter('.'+value);

      items.not('.'+value).removeClass('dd-case-studies__items__item--margin dd-case-studies__items__item--visible').hide(400);
      match.filter(':odd').addClass('dd-case-studies__items__item--margin');
      match.filter(':even').removeClass('dd-case-studies__items__item--margin');
      match.filter('.'+value).addClass('dd-case-studies__items__item--visible');
