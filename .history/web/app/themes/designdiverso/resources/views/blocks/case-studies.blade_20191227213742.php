...
<div class="dd-case-studies__items flex flex-wrap">
...
@endphp
  <div class="dd-case-studies__items__item">
    <h3>{{ $title }}</h3>
    <img src="{{ $image }}" alt="{{ $title }}">
    <a class="filter-button inline-block mt-4 text-black font-proxima" href="{{ $link }}">{{ __('View ', 'sage') . $category_singular_name }}</a>
  </div>
@endwhile
</div>
</section>
...
jQuery(document).ready(function(){
  jQuery(".filter-button").click(function(){
      ...
      $(".dd-case-studies__items").not('.'+value).hide('3000');
      $('.dd-case-studies__items').filter('.'+value).show('3000');
  });
});
</script>
